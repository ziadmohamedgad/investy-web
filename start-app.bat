...
start "Investment API Backend" powershell -NoProfile -NoExit -Command "Set-Location '%ROOT_DIR%src\Investment.API'; $host.UI.RawUI.WindowTitle = 'Investment API'; Write-Host 'Starting API on port 5091...' -ForegroundColor Green; dotnet run --launch-profile http"
...
start "Investment Web Frontend" powershell -NoProfile -NoExit -Command "Set-Location '%ROOT_DIR%src\Investment.Web'; $host.UI.RawUI.WindowTitle = 'Investment Web'; Write-Host 'Starting Angular UI on port %ANGULAR_PORT%...' -ForegroundColor Cyan; npm start -- --port %ANGULAR_PORT% --no-open"
