@echo off
title Investment Portfolio Tracker Launcher
...
start "Investment API Backend" powershell -NoProfile -NoExit -Command "Set-Location $env:API_DIR; $host.UI.RawUI.WindowTitle = 'Investment API'; Write-Host 'Starting API on port 5091...' -ForegroundColor Green; dotnet run --launch-profile http"
...
start "Investment Web Frontend" powershell -NoProfile -NoExit -Command "Set-Location $env:WEB_DIR; $host.UI.RawUI.WindowTitle = 'Investment Web'; Write-Host 'Starting Angular UI on port %ANGULAR_PORT%...' -ForegroundColor Cyan; node (Join-Path $env:WEB_DIR 'node_modules/@angular/cli/bin/ng.js') serve --port %ANGULAR_PORT% --no-open"
...
echo   To stop the app, simply close those windows.
echo ==============================================
